...
`AMA_EMBEDDING_PROVIDER=openai` and `OPENAI_API_KEY` first. Provider settings
are fixed for the lifetime of the process, so restart the API after changing
them.
...
database. Keep it bound to the loopback interface. Containerisation and its
runtime health check should be added separately so Docker remains an
